...
This vignette will detail an example run of TITAN for only 2 clonal clusters of chromosome 2 of a triple negative breast cancer sample [@Ha2012,Ha2013]. Once again, for real data, users should repeat the *TITAN* for additional clonal clusters and then select the optimal run using the minimum S\_Dbw Validity index.
...
\begin{enumerate}
  ...
  \item The read depth are normalized for GC content and mappability biases using the *HMMcopy* R package. *TITAN* uses a wrapper for the function in *HMMcopy* to accomplish this.
  \item *TITAN* requires as input, the read counts and normalized read depth to estimate model parameters and infer subclonal CNA/LOH.
  \item For real data, repeat *TITAN* analysis for a range of clonal clusters. 
\end{enumerate}
...
# Extracting tumour and normal read depth
...
\begin{itemize}
  ...
  \item \verb|s|: Converged estimate(s) for cellular prevalence parameter(s). This value is defined as the proportion of tumour sample that does *not* contain the aberrant genotype. This will contrast what is output in \verb|outputTitanResults|. \verb|numeric array| containing estimates at each EM iteration. If more than one cluster is specified, then \verb|s| is a \verb|numeric matrix|.
...
## Find the genotype and clonal cluster state paths
...
### Position-specific Copy Number Results
...
The correction is cmoputed based on the following formula from Ref[@Ha2013] using observed log ratio $l_t$ at position or segment $t$ and tumor purity $(1-n)$ and tumor ploidy $\phi$: 
...
One of the key features of *TITAN* is the estimation of cellular prevalence and the inference of clonal cluster membership. The estimated normal proportion is required for this plot. This can be obtained from \verb|convergeParams|.
